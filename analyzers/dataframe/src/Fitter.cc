...
struct AnalyticFunctor {
  ...
  double operator()(double *x, double *par) {
    ...
    double W = 1. / 3. *
               ((5 - 9 * xx * xx + 4 * xx * xx * xx) +
                P * (1 - 9 * xx * xx + 8 * xx * xx * xx));
...
myFit fit_fraction(const std::string &infile_data,
  ...
  std::cout << "[Fitter] Starting Fraction Fit (N+, N-) for " << plot_title
            << std::endl;
  ...
  auto h_data_ptr = df.Histo1D(
      {"h_data_frac", (plot_title + ";" + x_axis_title + ";Events").c_str(),
       nBins, xMin, xMax},
      dataColName);
  ...
  TFitResultPtr fitStatus =
      h_data->Fit(f_fit, "L S Q"); // S needed for Covariance Matrix access
  ...
  double var_P = (dP_dNp * dP_dNp * eNp * eNp) + (dP_dNm * dP_dNm * eNm * eNm) +
  ...
  std::cout << "[Fitter] " << plot_title
            << " (Fraction N+/N-) | P_tau: " << result.P_tau << " +/- "
            << result.P_err << std::endl;
...
myFit fit_no_plot(const std::string &infile_data,
  ...
  h_plus = (TH1D *)h_plus->Clone("mc_plus_noplot");
  h_minus = (TH1D *)h_minus->Clone("mc_minus_noplot");
  h_plus->SetDirectory(0);
  h_minus->SetDirectory(0);
  fTemp->Close();
  ...
  auto h_data_ptr = df.Histo1D(
      {"h_data_noplot", "Data;x;Events", nBins, xMin, xMax}, dataColName);
  TH1D *h_data = (TH1D *)h_data_ptr->Clone("data_noplot");
  h_data->SetDirectory(0);
  h_data->Sumw2();

  // linear fit with functor
  TemplateFunctor functor(h_plus, h_minus);
  TF1 *f_fit = new TF1("f_template_noplot", functor, xMin, xMax, 2);
...
myFit fit(const std::string &infile_data, const std::string &infile_templates,
  ...
  auto h_data_ptr = df.Histo1D(
      {"h_data", (plot_title + ";" + x_axis_title + ";Events").c_str(), nBins,
       xMin, xMax},
      dataColName);
  ...
  std::cout << "[Fitter] " << plot_title
            << " (Template TF1) | P_tau: " << result.P_tau << " +/- "
            << result.P_err << std::endl;
...
myFit fit(const std::string &infile_data, const std::string &outdir,
  ...
  auto h_ptr = df.Histo1D(
      {"h_data", (plot_title + ";" + x_axis_title + ";Events").c_str(), 40, 0.0,
       1.0},
      dataColName);
