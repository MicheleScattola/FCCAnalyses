...
RVec<myEvent> myget_event(const RVec<int> &mu_ids, const RVec<int> &el_ids,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int i = 0; i < mc.size(); i++) {
      ...
      // choose tau charge based on charge of hemisphere, in order they should
      // be : POS , NEG DO NOT RELY ON RECO CHARGES, BIASED!!!
    ...
    if (ev.mc_weight_plus < 0.0 || ev.mc_weight_minus < 0.0) {
      cerr << "[WARNING]: negative weights, setting reco type to 0 from "
           << ev.m_type << endl;
      ev.m_type = 0;
      ev.mc_type = 0;
    }
...
int classify_lep(myEvent &ev, const RVec<int> &mu_idx, const RVec<int> &el_idx,
  ...
  if (ev.n_mu == 0 && ev.n_el == 1) {
    ...
    // check RP2MC and get corresponding 1prong true MC energy
    int id_el = el_idx[0];
    ev.mc_RP2MC_id = rp2mc_idx[id_el];
...
// ==========================================
// return reco event type safely with mass check
...
// calculate P_tau costheta dependent
double calc_Ptau(const TLorentzVector &p4_tau) {
  double costheta = p4_tau.CosTheta();
  double Ptau =
      -(SM_Atau * (1 + costheta * costheta) + 2 * SM_Atau * costheta) /
...
void pion_weight(myEvent &ev, const RVec<edm4hep::MCParticleData> &mc,
  ...
  for (int i = pb; i < pe; i++) {
    ...
    if (abs(dau.PDG) == 211) {
      ...
      p_temp.SetXYZM(dau.momentum.x, dau.momentum.y, dau.momentum.z, dau.mass);
      p4_pi_lab += p_temp;
    }
  }
...
void rho_weight(myEvent &ev, const RVec<edm4hep::MCParticleData> &mc,
  ...
  ev.mc_omega =
      geometric_omega_rho(ev, p4_tau_lab, p4_rho_lab, p4_pi_lab, p4_pi0_lab);
}
...
double geometric_omega_rho(myEvent &ev, const TLorentzVector &p4_tau,
  ...
  if (cos_psi_tau > 1.01 || cos_psi_tau < -1.01) {
    cerr << "[WARNING]: cos(psi_tau) out of bounds: " << cos_psi_tau << endl;
    if (ev.m_type == 4)
      ev.m_debug = 99;
  }
  ...
  if (cos_psi_rho > 1.01 || cos_psi_rho < -1.01) {
    cerr << "[WARNING]: cos(psi_rho) out of bounds: " << cos_psi_rho << endl;
    if (ev.m_type == 4)
      ev.m_debug = 99;
  }
...
RVec<double> get_dressed_e(const RVec<myEvent> &evs, const int mc_type,
  ...
  for (const auto &e : evs) {
    ...
    if (masscheck && e.m_debug_mass == 1)
      continue;
    // collect dressed energy (if the event is mu or el the other P4 vectors are
    // empty)
    double temp = 0.0;
...
RVec<double> get_hadron_e(const RVec<myEvent> &evs, const int mc_type,
  ...
  for (const auto &e : evs) {
    ...
    // if reco && mc type are <0 collect any energy where reco != mc given a
    // said reco decay
