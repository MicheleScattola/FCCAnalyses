...
void fitLep_test() {
    ...
    TH1D* h_el_test = (TH1D*)f->Get("mc_el_test");
	TH1D* h_mu_test = (TH1D*)f->Get("mc_mu_test");

	TH1D* h_MCel = (TH1D*)f->Get("mc_el_all");
	TH1D* h_MCmu = (TH1D*)f->Get("mc_mu_all");

	TH1D* h_el_x = (TH1D*)f->Get("mc_el_x");
	TH1D* h_mu_x = (TH1D*)f->Get("mc_mu_x");

    // Fit for electrons
    if (h_el_test) {
        TString el = "testEL";
        TString title = "Diagonal events + MC energy + inv mass limit : e channel;x_{e};Events";
    ...
    if (h_mu_test) {
        TString mu = "testMU";
        TString title = "Diagonal events + MC energy + inv mass limit : #mu channel;x_{#mu};Events";
