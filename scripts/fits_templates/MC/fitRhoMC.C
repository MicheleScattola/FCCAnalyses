...
void fitRhoMC() {
    ...
    const char* infile_templates = "/afs/cern.ch/user/s/scattola/FCCAnalyses/Ztautau/treemaker/MC/templates/templatesMC.root";
    ...
    TH1D *h_plus  = (TH1D*)fTemp->Get("h_template_rho_plus");
    TH1D *h_minus = (TH1D*)fTemp->Get("h_template_rho_minus");
    ...
    auto h_data_ptr = df.Histo1D({"h_data", "Fit Polarization;x_{#rho};Events", nBins, xMin, xMax}, "rho_sgn");
    ...
    c->SaveAs(TString(outdir) + "TrueMC_rho_fit.pdf");
